...
function makeWord(lettersObject) {
  const letterArr = [];
  Object.entries(lettersObject).map((e) => {
    const [key, value] = e;
    value.map((index) => {
      letterArr[index] = key;
      return 0;
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    if (queue[i] === ticketPrice) {
      moneyEarned += queue[i];
    } else if (moneyEarned + ticketPrice - queue[i] < 0) {
      canSell = false;
      break;
    } else {
      moneyEarned = moneyEarned + queue[i] - (queue[i] - ticketPrice);
    }
  }
  return canSell;
}
...
function fromJSON(proto, json) {
  ...
  Object.entries(JSON.parse(json)).map((e) => {
    const [key, value] = e;
    obj[key] = value;
    return value;
  });
